...
object BatteryOptimizationHelper {
    ...
    fun agresifPilYonetimiOlabilirMi(): Boolean {
        ...
        return listOf("xiaomi", "redmi", "poco", "huawei", "honor", "oppo", "realme", "vivo", "oneplus", "meizu", "asus", "samsung")
    ...
    fun ureticiyeOzelAyarlariAc(context: Context) {
        val denenecekIntentler = otomatikBaslatmaIntentleri()
    ...
    /**
     * Üreticiye özel "Pil kullanımı / Kısıtlama yok" ayar ekranını açmayı dener. Bu,
     * otomatik başlatma izninden AYRI bir ayardır: bazı cihazlarda uygulama otomatik
     * başlatma iznine sahip olsa bile, pil kullanım profili "Kısıtlı" ya da "Optimize
     * edilmiş" olarak kalırsa sistem yine de arka plan işlerini (bildirimler dahil)
     * durdurabilir. Genelde "Uygulama bilgisi > Pil" yolunda bulunur; kullanıcının
     * burada "Kısıtlama yok" / "Sınırsız" seçeneğini seçmesi gerekir.
     */
    fun pilKullanimiKisitlamasiniAc(context: Context) {
        val denenecekIntentler = pilKullanimiIntentleri()
        ...
        if (!acildiMi) {
            // Üreticiye özel pil kullanımı ekranı bulunamadıysa, uygulamanın kendi
            // "Uygulama bilgisi" sayfasına düş; oradan da Pil sekmesine elle gidilebilir.
            uygulamaAyarlariniAc(context)
        }
    }

    private fun otomatikBaslatmaIntentleri(): List<Intent> {
        val uretici = Build.MANUFACTURER.lowercase()
        return when {
    ...
    /**
     * Üreticiye özel "pil kullanımı / kısıtlama" ekranını hedefleyen bilinen intent'ler.
     * Bu ekranlar genelde uygulamanın paket adını bir Uri/extra olarak ister; extra
     * isimleri üretici sürümüne göre değiştiği için burada sadece ekrana genel giriş
     * (uygulama listesi) hedeflenir, kullanıcı listeden kendi uygulamasını seçer.
     */
    private fun pilKullanimiIntentleri(): List<Intent> {
        val uretici = Build.MANUFACTURER.lowercase()
        return when {
            uretici.contains("xiaomi") || uretici.contains("redmi") || uretici.contains("poco") -> listOf(
                // MIUI "Pil ve performans > Uygulama kullanımı" ekranı
                Intent().setClassName(
            ...
            uretici.contains("huawei") || uretici.contains("honor") -> listOf(
                // EMUI/Magic UI "Pil > Uygulama başlatma" (manuel yönetim: arka plan çalışma açık)
                Intent().setClassName(
                    "com.huawei.systemmanager",
            ...
            uretici.contains("oppo") || uretici.contains("realme") -> listOf(
                ...
                Intent().setClassName(
                    "com.coloros.safecenter",
                    "com.coloros.safecenter.permission.PermissionMainActivity"
                )
            )
            uretici.contains("vivo") -> listOf(
                Intent().setClassName(
                    "com.iqoo.secure",
                    "com.iqoo.secure.ui.phoneoptimize.BackgroundManagerActivity"
                ),
                Intent().setClassName(
                    "com.vivo.abe",
            ...
            uretici.contains("samsung") -> listOf(
                // Samsung "Pil > Arka planda kısıtlanmayan uygulamalar" ekranı
